...
public class TestUtils {
    ...
    public static <T> T retrieveResourceFromResponse(HttpResponse response, Class<T> clazz) throws IOException {
        ...
        try {
            ...
            LOGGER.error("Error parsing response JSON", t);t.printStackTrace();
        }
        return null;
    }
    ...
    public static RequestResponse executeContextJSONRequest(HttpPost request, String sessionId) throws IOException {
        try (CloseableHttpResponse response = HttpClientBuilder.create().build().execute(request)) {
            ...
            if (sessionId != null) {Assert.assertEquals("Context sessionId should be the same as the sessionId used to request the context", sessionId,
